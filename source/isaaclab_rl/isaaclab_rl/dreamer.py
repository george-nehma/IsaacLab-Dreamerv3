"""Wrapper to configure an environment instance to DreamerV3 vectorized environment.

The following example shows how to wrap an environment for DreamerV3:

.. code-block:: python

    from isaaclab_rl.dreamer import DreamerVecEnvWrapper

    env = DreamerVecEnvWrapper(env)
...
# def process_dreamer_cfg(cfg: dict) -> dict:
...
class DreamerEnvWrapper(Env):
    ...
    def __init__(self, env: ManagerBasedRLEnv | DirectRLEnv):
        ...
        # initialize env
        Env.__init__(self, self.num_envs, observation_space, action_space)
